...
import (
	...
	// decor2 "github.com/DeathKing/mpb@marquee-decorator"
...
func main() {
	// parse input
	bit := flag.Int("bit", 1280, "block size")
	base := flag.Int("base", 32, "base32(32)/base64(64)")
	force := flag.Bool("force", false, "overwrite file")
	encodeName := flag.Bool("enc_name", false, "do not encode/decode file name")
	deleteOrigin := flag.Bool("delete", false, "delete origin file")
	flag.Parse()
	files := flag.Args()[1:]
	if len(files) == 0 {
		fmt.Printf("A tool reverse bytes bloks of files\n")
		fmt.Printf("  input : <block1><block2><block3>...\n")
		fmt.Printf("  output: <1kcolb><2kcolb><3kcolb>...\n")
		fmt.Printf("Usage: gotool reverse [flags] <file1 file2 ...>\n")
		fmt.Printf("Flags:\n")
		flag.PrintDefaults()
		return
	}
	...
	for i := 0; i < len(files); i++ {
		...
		if *deleteOrigin {
			...
			if err != nil {
				fmt.Printf("[error] file %v cannot be removed err=%v", files[i], err)
			}
		}
	}
